...
download model: http://dl.yf.io/drn/
"""
...
model_urls = {
    ...
    'drn-c-26': webroot + 'drn_c_26-ddedf421.pth',  # 无法下载
...
class DRN(nn.Module):
    ...
    def __init__(self, block, layers, arch='D',
        ...
        if arch == 'C':
            self.conv1 = nn.Conv2d(3, channels[0], kernel_size=7, stride=1,
                                   padding=3, bias=False)  # 7x7 16
            self.bn1 = BatchNorm(channels[0])
            self.relu = nn.ReLU(inplace=True)

            self.layer1 = self._make_layer(  # 3x3 16, 1
                BasicBlock, channels[0], layers[0], stride=1, BatchNorm=BatchNorm)
            self.layer2 = self._make_layer(  # 3x3 32, 1
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)  # 1, stride=1
        x = self.layer2(x)  # 1/2
        x = self.layer3(x)
        low_level_feat = x  # 1/4

        x = self.layer4(x)  # 1/8
        x = self.layer5(x)  # 之后 stride 都 = 1
        ...
        return x, low_level_feat  # 1/8, 1/4
...
from model.sync_batchnorm.batchnorm import SynchronizedBatchNorm2d
from model.decoder import MaskHead


class DRNSeg(nn.Module):
    """ https://github.com/fyu/drn/blob/master/segment.py   -s: --crop-size
    ...
    def __init__(self, arch, num_classes, pretrained=True,
        ...
        if arch == 'drn_a_50':
            self.base = drn_a_50(BatchNorm, pretrained)
        elif arch == 'drn_c_26':
            self.base = drn_c_26(BatchNorm, pretrained)
        elif arch == 'drn_d_22':
            self.base = drn_d_22(BatchNorm, pretrained)
        elif arch == 'drn_d_54':
            self.base = drn_d_54(BatchNorm, pretrained)
        else:
            raise NotImplementedError

        # last conv
        self.seg = nn.Conv2d(512, num_classes, kernel_size=1, stride=1, bias=True)  # 使用 bias
        self._init_weight(self.seg)

        # mask conv
        self.with_mask = with_mask
        if self.with_mask:
            self.mask_head = MaskHead(num_classes, BatchNorm)  # out_chan = 1
            self.zero_thre = 0.
            self.one_thre = 1.

    def forward(self, input):
        x, _ = self.base(input)  # 1/8
        x = self.seg(x)  # 1/8 x

        if self.with_mask:
            mask, attention = self.mask_head(x)
            x = F.interpolate(x, size=input.size()[2:], mode='bilinear', align_corners=True)
            mask = F.interpolate(mask, size=input.size()[2:], mode='bilinear', align_corners=True)  # nearest can't use align_corners
    ...
    def _init_weight(self, m):  # init seg
        n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
        m.weight.data.normal_(0, math.sqrt(2. / n))
        m.bias.data.zero_()

    def set_onezero_thres(self, one, zero):
        self.one_thre = one
        self.zero_thre = zero

    def get_lr_params(self):  # conv, bn
        modules = [self.base, self.seg]
        if self.with_mask:
...
if __name__ == "__main__":
    input = torch.rand(1, 3, 688, 688)
    model = DRNSeg('drn_d_54', 19, pretrained=True, sync_bn=False, with_mask=True)
    model.eval()
