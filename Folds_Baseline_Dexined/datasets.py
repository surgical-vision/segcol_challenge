...
class TestDataset(Dataset):
        ...
        if self.test_data == "CLASSIC":
            ...
            if self.test_data.upper()=='BIPED':
                ...
                for pair in pairs:
                    tmp_img = pair[0]
                    tmp_gt = pair[1]
                    if self.test_data == 'SEGCOL':
                        sample_indices.append(
                            (os.path.join(self.data_root, 'valid', tmp_img),
                             os.path.join(self.data_root, 'valid', tmp_gt),))
                    else:
                        sample_indices.append(
                            (os.path.join(self.data_root, tmp_img),
                            os.path.join(self.data_root, tmp_gt),))
    ...
    def __getitem__(self, idx):
        ...
        if self.test_data == 'SEGCOL':
            img_name = image_path.split('valid/')[1]
        else:
        ...
        elif self.test_data == 'SEGCOL':
            img_dir = os.path.join(self.data_root, 'valid')
            gt_dir = os.path.join(self.data_root, 'valid')
        ...
        if not self.test_data == "CLASSIC":
            ...
            #TODO: turn label to channels
            # Create class channels using np.where
            class1 = (np.where(label == 255, 1, 0)*255).astype("uint8") # fold
            # class2 = (np.where(img_data == 127, 1, 0)*255).astype("uint8") # tool1 
            # class3 = (np.where(img_data == 128, 1, 0)*255).astype("uint8") # tool2
            # class4 = (np.where(img_data == 129, 1, 0)*255).astype("uint8") # tool3
...
class BipedDataset(Dataset):
    ...
    def _build_index(self):
        ...
        if self.arg.train_data.lower()=='biped':
            ...
            if self.arg.train_data.lower()=='bsds' or self.arg.train_data=='SEGCOL':
                ...
                for pair in pairs:
                    tmp_img = pair[0]
                    tmp_gt = pair[1]
                    if self.arg.train_data == 'SEGCOL':
                        sample_indices.append(
                            (os.path.join(self.data_root, 'train', tmp_img),
                             os.path.join(self.data_root, 'train', tmp_gt),))
                    else:
                        sample_indices.append(
